...
create proc spinsertworkareas @skill nvarchar(30), @id int output
as
declare @wid int
select @wid = isnull(max(workareaid), 0)+1 from workareas
begin try 
	insert into workareas(workareaid,skill) values(@wid,  @skill)
	set @id = @wid
	return 0
end try
begin catch
     declare @msg nvarchar(1000), @eno int
	 select @msg = error_message(), @eno = error_number()
	 ;
	 throw 50001, @msg, 1
	 return @eno
end catch
...
go
create function fnworksbyid(@wid int ) returns table
as
return (
...
go
create function fnworksbyworkername(@n nvarchar(40)) returns table
as
return
(
